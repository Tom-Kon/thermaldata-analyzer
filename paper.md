...
# Statement of need
TRIOS does allow users to save analysis templates and report templates for rapid analysis of similar samples. It also permits exporting data to Excel, but this process is less automated and customizable [@TRIOS:2024]. Consequently, issues arise when analyzing similar samples in scientific research contexts, particularly when dealing with replicates (runs) as TRIOS treats them as entirely separate files. This complicates the automatic export of data from different runs to Excel. Furthermore, difficulties are exacerbated when researchers need to calculate descriptive statistics such as mean, standard deviation, and relative standard deviation, despite these being straightforward operations.
